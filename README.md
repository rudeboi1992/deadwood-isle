...
Everything below is server-side. You don't need to install anything except the one Workshop mod, which Steam handles for you on join.

### Loot
- **2x loot.** Twice as many item spawners are active (40% vs 20%) and the world holds twice as many items at once (1,500 vs 750).
...
- **Axes out-gather hatchets.** Hatchets now yield on every swing; axes yield double per swing.
